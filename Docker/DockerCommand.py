...
#? 1nci yöntem frontend ve backend klasörleri içine ayrı ayrı İMAGE ve CONTAINER oluşturma;
- dockerignore oluştur, (aynı olabilir)
- dockerfile oluştur,
...
-8000:8000 localimdeki ile dockerdaki portları eşleştiriyorum, 8001:8000 de olur,
...
# aslında sadece 
docker run nameofIMAGE, bu bir CONTAINER ayağa kaldırır fakat isimsiz olur ve bir port bağlantısı olmaz,


# data'lar kalıcı değilse IMAGE'de ne varsa öyle gelir yani;
bir CONTAINER oluşturup veri girişi yapıldıktan sonra aynı IMAGE'den yeni bir CONTAINER oluşturulduğunda önceden girilen veriler görünmez, (eğer kalıcı hale getirilmediyse)
veya frontende bir değişiklik yapıldığında görünmesi için değişiklikten sonra yeni bir IMAGE oluşturup, ondan yeni bir CONTAINER ayağa kaldırmak gerekir.


#? 2nci yöntem tek bir DOCKER-COMPOSE dosyası ile yapma;
...
docker da oluşturduğum CONTAINER içindeki (backend) kodlar ile eşleşti, artık anlık değişiklikler görünecek ve kalıcı olacak.
